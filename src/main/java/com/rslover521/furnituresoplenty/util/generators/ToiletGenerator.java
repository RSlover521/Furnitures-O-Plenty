...
public class ToiletGenerator {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("✅ Toilet JSONs generated for all wood types!");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static void generateBlockstate(Path output, String wood) throws IOException {
        ...
        {
          "variants": {
            "facing=east":  { "model": "%MODID%:block/%WOOD%_toilet", "y": 90 },
            "facing=north": { "model": "%MODID%:block/%WOOD%_toilet" },
            "facing=south": { "model": "%MODID%:block/%WOOD%_toilet", "y": 180 },
            "facing=west":  { "model": "%MODID%:block/%WOOD%_toilet", "y": 270 }
          }
        }
        """.replace("%MODID%", MOD_ID)
           .replace("%WOOD%", wood);

        Path path = output.resolve("assets/" + MOD_ID + "/blockstates/" + wood + "_toilet.json");
    ...
    private static void generateBlockModel(Path output, String wood) throws IOException {
        String json = """
        {
          "parent": "%MODID%:block/toilet",
          "textures": {
            "particle": "biomesoplenty:block/%WOOD%_planks",
            "texture": "%MODID%:block/%WOOD%_toilet"
          }
        }
        """.replace("%MODID%", MOD_ID)
           .replace("%WOOD%", wood);

        Path path = output.resolve("assets/" + MOD_ID + "/models/block/" + wood + "_toilet.json");
        Files.createDirectories(path.getParent());
        Files.writeString(path, json);
    }
    ...
    private static void generateItemModel(Path output, String wood) throws IOException {
        String json = """
        {
          "parent": "%MODID%:block/%WOOD%_toilet"
        }
        """.replace("%MODID%", MOD_ID)
           .replace("%WOOD%", wood);

        Path path = output.resolve("assets/" + MOD_ID + "/models/item/" + wood + "_toilet.json");
        Files.createDirectories(path.getParent());
        Files.writeString(path, json);
    }
}
